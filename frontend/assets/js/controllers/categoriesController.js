            ...
            modal.innerHTML = `
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg w-full max-w-4xl max-h-screen overflow-y-auto">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Articles in "${category.name}"</h2>
                    ...
                    <div class="space-y-4">
                        ${articles.length === 0 ? 
                            '<p class="text-gray-500 dark:text-gray-400 text-center py-8">No articles in this category</p>' :
                            articles.map(article => `
                                <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                    <h3 class="font-semibold text-gray-800 dark:text-white mb-2">${article.title}</h3>
                                    <p class="text-gray-600 dark:text-gray-300 text-sm mb-2">${article.content.substring(0, 150)}...</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">By ${article.author || 'Anonymous'} • ${this.formatDate(article.created_at)}</p>
                                </div>
                            `).join('')
                        }
                    </div>
